...
* **🗣️ Speech-to-Text Service:** converts the caller’s voice to text (so that it can be passed to LLMs) and understands speech patterns such as when the user is done speaking and interruptions to facilitate interruptibility (ElevenLabs Scribe v2 Realtime)
...
### Real-time Strategy
For details on the sub–500 ms response plan and production-ready interruption handling, read [`docs/real_time_strategy.md`](docs/real_time_strategy.md).

### Twilio Limits
Bulk and single call launches respect Twilio's published call-creation rate limits. See [`docs/twilio_call_limits.md`](docs/twilio_call_limits.md) for the exact numbers and how to tune them.
...
Please note that you can choose between `anthropic` and `openai` for the LLM service. Speech-to-text **and** text-to-speech are now exclusively powered by ElevenLabs’ realtime stack (Deepgram has been removed).
...
TWILIO_CALL_THROTTLE_SEC=1.0   # see docs/twilio_call_limits.md for CPS guidance
...
RECORD_CALLS=false
LLM_RESPONSE_BUDGET_SEC=0.45   # force first audio chunk under 500ms
INTERRUPT_DEBOUNCE_SEC=0.5      # ignore duplicate barge-ins within this window
TRANSCRIPT_DUPLICATE_WINDOW_SEC=2.0  # suppress duplicate STT finals for smoother turns
ASSISTANT_DUPLICATE_WINDOW_SEC=1.5   # prevent the agent from replaying the same chunk twice
TRANSCRIPT_FLUSH_SEC=0.35            # forces stale transcripts to flush quickly
ELEVENLABS_STT_MODEL_ID=scribe_v2_realtime
ELEVENLABS_STT_AUDIO_FORMAT=pcm_16000
ELEVENLABS_STT_SAMPLE_RATE=16000
ELEVENLABS_STT_COMMIT_STRATEGY=vad   # use ElevenLabs server-side VAD for <500ms turns
ELEVENLABS_STT_VAD_THRESHOLD=0.4
ELEVENLABS_STT_VAD_SILENCE_THRESHOLD=0.6
ELEVENLABS_STT_MIN_SPEECH_MS=120
ELEVENLABS_STT_MIN_SILENCE_MS=120
ELEVENLABS_STT_MAX_RECONNECTS=3
ELEVENLABS_STT_RECONNECT_DELAY_SEC=0.5
```
